    ...
    for critic_lr in config['hmrd_ppo']['critic_lr_list']:
        for cdfc_actor_lr in config['hmrd_ppo']['cdfc_actor_lr_list']:
            for cfc_actor_lr in config['hmrd_ppo']['cfc_actor_lr_list']:
                ...
                simulation.level = 3 # set training scenario level
                simulation.init_level() # initialize
                ...
                cfc_rewards = cfc_mean_rewards_parallel(3, config_path, cdfc_state, cfc_state,
                                                              camera_range, simulation.level)

                high_cdfc_reward = cdfc_rewards
                high_cfc_reward = cfc_rewards
                ...
                for episode in range(config['hmrd_ppo']['episodes']):
                    if episode % 30 == 0 and episode != 0:
                        ...
                        for epi in range(10):
                            if epi == 0:
                                simulation.update_launch_pads(best_cdfc)
                                simulation.update_cameras(cfc_state)
                                cdfc_rewards, cfc_rewards, cdfc_scores, cfc_scores, dump_float = simulation.simulate()
                                # PARALLEL CALL
                                cfc_rewards = cfc_mean_rewards_parallel(3, config_path, best_cdfc, cfc_state,
                                                                              camera_range, simulation.level)
                            ...
                            cfc_state, cfc_value, cfc_monitor = cfc_agent.select_action(
                                state_tensor.flatten(), epi_index, cfc_monitor, i=-1, j=3)
                            ...
                            if cfc_rewards > high_cfc_reward:
                                best_cfc = cfc_state
                                high_cfc_reward = cfc_rewards

                            next_state = flattened_cdfc_scores + flatten_cfc_state
                            ...
                            rewards = cdfc_rewards + cfc_rewards

                            cfc_episodes_log.append(epi_index)
                            cfc_rewards_log.append(cfc_rewards)
                            if len(cfc_rewards_log) >= 50:
                                avg_reward = sum(cfc_rewards_log[-50:]) / 50
                                cfc_avg_reward_log.append(avg_reward)
                                print(
                                    f"CFC Episode {epi}, CFC Reward: {cfc_rewards}, Avg Last 50: {avg_reward}")
                            else:
                                avg_reward = sum(cfc_rewards_log) / len(cfc_rewards_log)
                                print(f"CFC Episode {epi}, CFC Reward: {cfc_rewards}")
                                cfc_avg_reward_log.append(avg_reward)
                            cfc_agent.train(state_tensor, next_state_tensor, rewards, cfc_value,
                                               cfc_next_value)
                            epi_index += 1
                        print("-----  Finish Training CFC  -----")

                    flattened_cdfc_scores = flatten_list(cdfc_scores)
                    flatten_cfc_state = flatten_list(best_cfc)
                    current_state = flattened_cdfc_scores + flatten_cfc_state
                    ...
                    simulation.update_cameras(best_cfc)
                    ...
                    if len(cdfc_rewards_log) >= 50:
                        avg_reward = sum(cdfc_rewards_log[-50:]) / 50
                        how_many_100 = capture_rate_log.count(100.0)
                        if capture_rate == 100:
                            print(f'!!!!!   Interception 100%   !!!!!')
                            print(f'Number of 100% = {how_many_100}')
                        cdfc_avg_reward_log.append(avg_reward)
                        print(
                            f"Episode {episode}, Reward: {cdfc_rewards}, Interception Rate: {capture_rate}, Avg Last 50: {avg_reward} Level {simulation.level}")
                    else:
                        avg_reward = sum(cdfc_rewards_log) / len(cdfc_rewards_log)
                        print(
                            f"Episode {episode}, Reward: {cdfc_rewards}, Interception Rate: {capture_rate}, Level {simulation.level}")
                        cdfc_avg_reward_log.append(avg_reward)

                    best_configs = save_best_config(best_configs, cdfc_state, best_cfc, cdfc_rewards,
